...
public class FirebaseAuthDatabase implements AuthDatabase {
    ...
    private Authentication authenticationFrom(FirebaseUser currentUser) {
        ...
        return new Authentication(new User(currentUser.getUid(), currentUser.getDisplayName(), userStatus == "" ? "" : userStatus.toString(), userPlaceName == "" ? "" : userPlaceName.toString(), userPlaceLat == "" ? "" : userPlaceLat.toString(), userPlaceLong == "" ? "" : userPlaceLong.toString(), photoUrl == null ? "" : photoUrl.toString(), currentUser.getEmail()));
    }
